...
from supervised.rubik import gen_rubik_data, hf_rubik_value
from utils import hf as hf_utils


class TrainHfForRubikValue(HfTrainingPipelineRubik):
    def __init__(
        self,
        n_proofs=10000,
        n_val_proofs=2,
        ...
        **kwargs
        ...
        super().__init__(
            tokenizer=hf_rubik_value.RubikValueTokenizer(
            ...
            model_config_overrides=hf_utils.ModelConfigOverrides(
                ...
                max_position_embeddings=hf_rubik_value.SEQUENCE_LENGTH,
                # min_length=hf_rubik_value.SEQUENCE_LENGTH,
                vocab_size=len(hf_rubik_value.VOCABULARY)),
            **kwargs
        )
        self.n_proofs = n_proofs
        self.n_val_proofs = n_val_proofs
        self.max_seq_length = hf_rubik_value.SEQUENCE_LENGTH
    ...
    def _log_validity_metrics(self, dataset, sequences, tokenizer, done_epochs, prefix, model=None):
        ...
        for entry, prediction in zip(dataset, sequences):
            ...
            distance_loss += sum([abs(ord(c1) - ord(c2)) for c1, c2 in zip(target_str, prediction)])
            st_accuracy += 1 if target_str.replace(hf_rubik_value.PADDING_LEXEME, '') == prediction.replace(hf_rubik_value.PADDING_LEXEME, '') else 0

        st_accuracy /= len(sequences)
        distance_loss /= len(sequences)

        log_scalar(f'{prefix} | statewise accuracy', done_epochs, st_accuracy)
        log_scalar(f'{prefix} | distance loss', done_epochs, distance_loss)

    def _generate_dataset(self, n_proofs, done_epochs, log_prefix):
        formula_pairs = gen_rubik_data.generate_value_learning_data(n_proofs)
